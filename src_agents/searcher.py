...
from config import TAVILY_API_KEY, GROQ_API_KEY, GROQ_API_KEY_2
...
class SearchAgent:
    ...
    def search_topic(self, query:str, search_type:str ="all")-> List:
        """
            Search the user topic on the web or 
            Args:
                query:str =  user query
                search_type:str = wiki or web search
        """
        sources = []
    ...
    def search(self, topic:str, sort_results=True):
        ...
        if sort_results:
            return sorted(all_sources['sources'], key=lambda x:x['score'], reverse=True)
        else:
            return all_sources
